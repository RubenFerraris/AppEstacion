	...
	*/
	...
	class User extends DBAbstract{
		...
		function __construct(){
			...
			$result = $this->query('DESCRIBE usuarios_appestacion');
		...
		function get_users(){
			$result = $this->query("SELECT 
			    (SELECT COUNT(*) FROM usuarios_appe
stacion) AS total_users, 
...
se ha validado, revise su casilla de correo", "errno" => 404];
...
$blockUrl = 'https://mattprofe.com.ar/alumno/6915/app-estacion/blocked?token='.$this->new_token($email);;
...
HTML;
		...
		function register($form){
			...
			$result = $this->query("SELECT * FROM usuarios_appestacion WHERE email = '$email'");
			...
			if(count($result)==0){
				...
				$ssql = "INSERT INTO usuarios_appe
stacion(token,email, password, token_action,add_date) VALUES ('$token','$email','$pass', '$token_email',NOW())";
...
<body style="font-family: Arial, sans-serif;">
    ...
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ccc; border-radius: 8px;">
        ...
        <p style="text-align: center;">
            <a href="https://mattprofe.com.ar/alumno/6915/app-estacion/verify?token='.$token_email.'" style="display: inline-block; padding: 12px 24px; background-color: #007bff; color: #fff; text-decoration: none; border-radius: 5px;">Click aquí para activar tu usuario</a>
        </p>
...
';
			...
			if(isset($result['delete_date'])){
				...
				$ssql = "UPDATE usuarios_appe
stacion SET first_name='', last_name='', `password`='$pass', delete_at='0000-00-00 00:00:00' WHERE id=$id";
		...
		function update($form){
			...
			$ssql = "UPDATE usuarios_appestacion SET active=1,token_action='',active_date=NOW() WHERE token_action='$token'";
		...
		function update_block($form){
			...
			$ssql = "UPDATE usuarios_appestacion SET blocked=1,token_action='',blocked_date=NOW() WHERE token_action='$token'";
		...
		function new_token($email){
			...
			$ssql = "UPDATE usuarios_appestacion SET token_action='$token' WHERE email='$email'";
		...
		function recovery($form){
			...
			$ssql = "SELECT * FROM usuarios_appestacion WHERE email='$email'";
			$result = $this->query($ssql);
			if ($result == null) {
				return ["error" => "Email no registrado", "errno" => 404];
			}
...
<body>
    <div class="container">
        ...
        <a href="https://mattprofe.com.ar/alumno/6915/app-estacion/reset?token='.$new.'" class="button">Click aquí para restablecer contraseña</a>
    </div>
</body>
</html>
';
			...
			$ssql = "UPDATE usuarios_appestacion SET recovery=1,recover_date=NOW() WHERE email='$email'";
			$result = $this->query($ssql);
		...
		function reset($form){
			...
			$pass = md5($form["txt_pass"].$_ENV['PROJECT_NAME']);
			$ssql = "SELECT * FROM usuarios_appestacion WHERE token_action='$token'";
			$result = $this->query($ssql);
			$email = $result[0]['email'];
			$ssql = "UPDATE usuarios_appestacion SET recovery=0,blocked=0,recover_date=NOW(),password='$pass' WHERE token_action='$token'";
			$result = $this->query($ssql);
			...
			$new = $this->new_token($email);
			// URL para bloquear la cuenta
			$blockUrl = 'https://mattprofe.com.ar/alumno/6915/app-estacion/blocked?token='.$new;
$cuerpo_email = <<<HTML
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    <div class="container">
        <h1>Información de Recuperacion de cuenta</h1>
